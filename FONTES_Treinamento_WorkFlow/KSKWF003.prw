...
Static Function KSKWFE03(aAprov,nRegSCR,cMensProg,cOpc,aMensIncon)
  ...
  Local cCondgto   := ''
  Local cContato   := ''
  Local cWfDiret   := SuperGetMV( 'KS_WFDIRE',,'\workflow\compras\' )
  Local aCabec     := {} 
  Local aItens     := {}                        
  Local nTotal     := 0
  Local oProcess   := Nil
  Local cHttpWf    :="localhost" //Alltrim(SuperGetMV("MV_HTTPWF",,"127.0.0.1"))//Local onde estarใo arquivos Workflows
  Local cPortaWF   := "91" //Alltrim(SuperGetMV("MV_HTTPPWF",,"91"))//Porta do http
...
//    oHtml:ValByName( 'COMPRADOR'        , UsrRetName(SC7->C7_USER) )
   ...
   If !"@" $ oProcess:cTO
    ...
    //     U_CONSOLE(_cTo)
      ...
      cCopyWF := '/messenger/emp' +cEmpAnt  + '/' + "compras" + '/' + alltrim(cProcess)+ '.htm' //temporario pois esta copiando no mensager e nใo no workflow
  ...
  //   	 __CopyFile(cCopyWF, '/workflow/http'+ '/' + alltrim(cProcess) + '.htm') // Realiza a copia do arquivo
	//oProcess:ohtml:valbyname('proc_link',cHostWF+'/workflow/messenger/'+'/emp'+ cEmpAnt + '/koala/' + cMailId + '.HTM' ) //Defino o Link onde foi gravado o HTML pelo Workflow,abaixo do diret๓rio do usuแrio definido em cTo do processo acima.
